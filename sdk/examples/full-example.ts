...
async function main(): Promise<void> {
  try {
    ...
    const advancedScanCommand: ZMapConfig = new ZMap()
      .tcpSynScan(443)
    ...
    console.log('\n--- EXAMPLE 3: ICMP Echo Scan ---');
    const icmpScanCommand: ZMapConfig = new ZMap()
      .icmpEchoScan()
      .target('8.8.8.8/24')
      .setMaxTargets(100)
    ...
    console.log('\n--- EXAMPLE 4: UDP Scan with Text Probe ---');
    const udpScanCommand: ZMapConfig = new ZMap()
    ...
    const outputOptionsCommand: ZMapConfig = new ZMap()
      .tcpSynScan(22)
    ...
    console.log('\n--- EXAMPLE 6: Logging and Metadata Options ---');
    const loggingCommand: ZMapConfig = new ZMap()
    ...
    console.log('\n--- EXAMPLE 7: Network Options ---');
    const networkCommand: ZMapConfig = new ZMap()
      .tcpSynScan(80)
      .target('10.0.0.0/24')
      .setSourcePort(12345)
    ...
    console.log('\n--- EXAMPLE 8: Additional Options ---');
    const additionalCommand: ZMapConfig = new ZMap()
      .tcpSynScan(80)
    ...
    console.log('\n--- EXAMPLE 9: Custom Probe Module ---');
    const customProbeCommand: ZMapConfig = new ZMap()
      .setConfig({
    ...
    const result: ZMapResult = await zmap
      .tcpSynScan(80)
    ...
    // Parse the results
    if (result.success) {
      const parsedResults: Record<string, string>[] = parseZMapOutput(result.output);
